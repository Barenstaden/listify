<template>
  <div>
    <Content class="py-10 text-white text-lg">
      <h1 class="text-center text-2xl lg:text-5xl">Om Listify</h1>
      <p class="mt-10">
        <strong
          >Listify er en handleliste som sorterer seg selv helt automatisk når
          du ankommer butikken. Den smarte handlelisten fungerer i alle butikker
          i hele Norge, og blir bedre for hver gang du bruker den.</strong
        >
      </p>
      <h2 class="text-xl lg:text-3xl mt-10">Automatisk sortering av varer</h2>
      <p class="mt-4">
        Listify lærer seg hvordan man handler, og bruker dette for å hjelpe deg
        med å sortere varene slik at du alltid er nærmest neste varekategori på
        listen.
      </p>
      <h2 class="text-xl lg:text-3xl mt-10">Ingen unødvendige funksjoner</h2>
      <p class="mt-4">
        Det skal ikke være nødvendig å måtte søke etter et produkt eller velge
        antall før du legger ting i handlelisten. Skriv helt enkelt hva du vil,
        så lenge du inkluderer navnet på varen du ønsker å legge til, så gjør
        Listify resten.
      </p>
      <h2 class="text-xl lg:text-3xl mt-10">
        Dekker alle butikker i hele Norge
      </h2>
      <p class="mt-4">
        Uansett om du bor i Arendal, Oslo, Trondheim, Alta eller på Gimsøya
        dekker vi din lokalbutikk så fort du begynner å handle der. Du vil
        kanskje merke at Listify, som alle andre, trenger litt tid på å bli
        kjent på nye steder. Er du den første som handler på butikken kan du
        klappe deg selv på skulderen - du har nemlig hjulpet neste person til å
        gjøre handleturen litt smidigere.
      </p>
      <h2 class="text-xl lg:text-3xl mt-10">
        Ingen andre trenger å vite hva eller hvor du har handlet
      </h2>
      <p class="mt-4">
        Listify er kun laget for å hjelpe deg å handle, ikke for å dele datene
        dine med noen andre. Vi må spørre deg om å dele posisjonen din for å
        avgjøre hvilken butikk du er i, og for å lære oss hvordan butikken du er
        i er organisert. Når du er ferdig med å handle og forlater butikken blir
        lokasjonen din automatisk slettet for godt.
      </p>
      <h2 class="text-xl lg:text-3xl mt-10">Åpenhet om teknologi</h2>
      <p class="mt-4">Har du spørsmål til hvordan Listify fungerer eller hvordan dataene dine blir håndtert bak kulissene? Still et spørsmål eller eller legg igjen en beskjed nede i høyre hjørne, så deler vi mer enn gjerne hvordan vi håndterer data og hvordan Lisify fungerer helt ned til kildekoden.</p>
    </Content>
  </div>
</template>

<script>
export default {};
</script>

<style></style>
